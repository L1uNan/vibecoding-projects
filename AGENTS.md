# Project Agent Instructions (OpenSpec / Hybrid Workflow)

This repo uses **OpenSpec** as a *file-based* workflow. In Codex, the workflow is enforced by the presence and upkeep of specific files, not by OpenCode-style slash commands.
...
`.opencode/commands/*.md` are OpenCode command templates. Codex does not auto-load these as slash commands; treat them as documentation only.
